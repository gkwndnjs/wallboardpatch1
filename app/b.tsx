...
export default function BScreen() {
  ...
  const handleCreateWall = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      // Get current device ID as creator ID
      ...
      const wallData: WallData = {
        ...
        creator_id: creatorId // Use device ID as creator ID
      };
  ...
  const deleteWall = async (shortId: string): Promise<void> => {
    try {
      ...
      // Get wall data to check creator
      const wallData = await getWallData(shortId);
      
      if (!wallData) {
        Alert.alert('오류', '담벼락을 찾을 수 없습니다.');
        return;
      }
      
      // Check if current user is the creator
      if (wallData.creator_id !== currentUserId) {
        Alert.alert('오류', '작성자만 담벼락을 삭제할 수 있습니다.');
        return;
      }
      ...
      Alert.alert('오류', '담벼락 삭제 중 오류가 발생했습니다.');
    }
  };
